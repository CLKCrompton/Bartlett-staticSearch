                ...
                <html lang="en" xml:lang="en">   
                    ...
                    <body> <!-- We'll need a menu here -->
                        ...
                        <xsl:for-each select="persName">
                            ...
                            <xsl:variable name="birthPlace" select="tokenize(../birthPlace, ',\s*')"/>
                            <xsl:variable name="occupation" select="tokenize(../occupation, ',\s*')"/>
                            ...
                            <p>Born in <xsl:value-of select="../birthYear"/> in <xsl:value-of select="concat($birthPlace[1], ', ', $birthPlace[last()])"/>, <xsl:value-of select="."/> was best known for contributions to <xsl:value-of select="../affiliation"/>. <xsl:value-of select="."/> had a number of occupations, including <xsl:value-of select="concat(string-join($occupation[position() &lt; last()], ', '), ' and ', $occupation[last()])"/><!-- For occupations: 1)tokenize <occupation> on ',\s*' to create a list. 2)Then, create an output when the items in the list are separated by a coma with "and" before the last item. 2a) Joins all the items from the list and separates them with a coma, except the last one. 2b) Concats the joined list, with 'and' and the last item in <occupation>. -->. The first instance of a quotation from <xsl:value-of select="."/> shows up in the <xsl:choose>
                            ...
                            </xsl:choose> edition of Bartlett’s Familiar Quotations. Find out more about this person in <xsl:element name="{'a'}" use-attribute-sets="href">Wikidata.</xsl:element></p>
