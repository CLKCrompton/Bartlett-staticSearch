<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="row">
        <xsl:for-each select="persName">
            ...
            <p>Born in <xsl:value-of select="../birthYear"/> in <xsl:value-of select="../birthPlace"/><xsl:comment>item-1-and2-from-commma-separated-cell-content(firstitem-adnlast-e.ge-towns-and-city)</xsl:comment> in <xsl:value-of select="../birthDate"/> <xsl:comment>cast-as-human-readable-date</xsl:comment>, <xsl:value-of select="."/><xsl:text> </xsl:text> was most famous in the fields of <xsl:value-of select="../affiliation"/>. <xsl:value-of select="."/> had a number of occupations, including <xsl:value-of select="../occupation"/>. A quotation from <xsl:value-of select="."/><xsl:text> </xsl:text> first shows up in the <xsl:value-of select="../firstAppearance"/><xsl:comment>cast-as-human-readable(e.g. first, fifth, twelfth)</xsl:comment>  edition of Bartlett’s familiar quotations. Find out more about this person in <xsl:element name="{'a'}" use-attribute-sets="href">Wikidata.</xsl:element></p>
